...
class Tank7Destroyer(
    ...
    private var tank7RecoilData = RecoilData(recoilAmount = 0.3f)

    // 이미지 로딩.
    //   Gdx.files.internal: 클래스패스(자원 폴더)에서 파일을 찾아 읽는다.
    //   Texture 는 GPU 메모리에 이미지를 올린 핸들이다.
    //   src/main/resources/player.png 에 위치.
    private val gun = Texture(Gdx.files.internal("tank_image/tank7_Destroyer/Destroyer_gun.png"))
    ...
    override fun update(delta: Float) {
        tank7RecoilData = recoil(tank7RecoilData.recoilTime,
            tank7RecoilData.recoilStrength,
            tank7RecoilData.recoilAmount,
            tankReloadSpeed,
    ...
    override fun draw(batch: SpriteBatch) {
        ...
        val yRecoil: Float = MathUtils.sin(angle * MathUtils.degreesToRadians) * tank7RecoilData.recoilStrength
